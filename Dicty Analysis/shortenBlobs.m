...
function [blob, frame2blob] = shortenBlob(N, blob, minDuration, maxRegionSize, minSolidity)
...
for b=1:length(blob)
    ...
    % if the blob has not been removed, add it to tempBlob and add its frames to frame2blob    
    if blob(b).removed == 0
        
        % update tempBlob
        tempBlob(freeID) = blob(b); 
