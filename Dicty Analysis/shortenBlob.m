...
function [blob, frame2blob] = shortenBlob(N, blob, minDuration, minRegionSize, maxRegionSize, minSolidity)
...
for b=1:length(blob)
    ...
    % remove blobs whose mean area is too small
    elseif mean(blob{b}.area) < minRegionSize 
        blob{b}.removed = 1;
    ...
    % if the blob has not been removed, add it to blob and add its frames to frame2blob    
    if blob{b}.removed == 0
