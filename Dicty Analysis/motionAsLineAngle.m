%%%%%%%% MOTION AS LINE ANGLE %%%%%%%%
% bins the shapes by orientation with respect to the lines, then finds the mean aligned motion, protrusion, and retraction    
function motionAsLineAngle(numBins, motionAreaThresh, M, shape, inDirectory, frameTime, pixelsmm, frameDelta, savePath)
    ...
    for t=1:length(shape(s).durationInROI) 
        ...
        if endFrame >= startFrame
            ...
            for b=1:numBins
                ...
                % (to use just replace binMask below with eccMask.*binMask in its 3 locations and binMaskSmall with eccMaskSmall.*binMaskSmall)
                % eccMaskSmall = (shape(s).eccentricity(startFrame:endFrame) >= 0.7344 & shape(s).eccentricity(startFrame:endFrame) <= 0.7492);
                % eccMask = repmat(eccMaskSmall', 1, (M-1)/2);
                eccSum(1,b) = eccSum(1,b)+sum(binMaskSmall.*motionMaskSmall.*shape(s).eccentricity(startFrame:endFrame));
...
for b=1:numBins
    ...
    eccMean(1,b) = eccSum(1,b)/count(b);
end
...
% for b=1:numBins
%     plot(shapeMean.motionFront(numBins-b+1,:)*convertAreaPerTime, 'LineWidth', 2, 'Color', colors(numBins-b+1,:))
%     hold on
% end
% colorbar;
% title('Local motion as a function of cell orientation with respect to the lines');
% xlabel('boundary position (a.u.) (front-left; back-right)');
% ylabel('local motion (square micrometers/minute)')
% 
% % find where the local motion plot intersects the x-axis
% [crossValue,crossBP] = min(abs(shapeMean.motionFront));
% 
% 
% % find the average speed
% meanSpeed = meanOnLine.inSpeed*convertSpeed;
% 
% plot(meanSpeed, crossBP, 'Marker', '.', 'MarkerSize', 20)
% hold on


% plot protrusions and retractions
figure; 
colors = colormap(hsv(numBins)); %colors = colormap(cool(2*numBins+1));
for b=1:numBins
...
colorbar;
title('Protrusive and retractive motion as a function of cell orientation with respect to the lines');
xlabel('boundary position (a.u.) (front-left; back-right)');
ylabel('local motion (square micrometers/minute)')

% % plot normalized (by probability) protrusions and retractions
% figure; 
% colors = colormap(hsv(numBins)); %colors = colormap(cool(2*numBins+1));
% for b=1:numBins
%     plot(shapeMean.protrusionFront(numBins-b+1,:)*convertAreaPerTime/sum(shapeMean.protrusionFront(numBins-b+1,:)*convertAreaPerTime), 'LineWidth', 2, 'Color', colors(numBins-b+1,:))
%     hold on
%     plot(-1*shapeMean.retractionFront(numBins-b+1,:)*convertAreaPerTime/sum(shapeMean.retractionFront(numBins-b+1,:)*convertAreaPerTime), 'LineWidth', 2, 'Color', colors(numBins-b+1,:))   
% end
% colorbar;
% title('Normalized protrusive and retractive motion as a function of cell orientation with respect to the lines');
% xlabel('boundary position (a.u.)');
% ylabel('normalized local motion (square micrometers/minute)')
% 
% % plot normalized (by max) protrusions and retractions
% figure; 
% colors = colormap(hsv(numBins)); %colors = colormap(cool(2*numBins+1));
% for b=1:numBins
%     plot(shapeMean.protrusionFront(numBins-b+1,:)*convertAreaPerTime/max(shapeMean.protrusionFront(numBins-b+1,:)*convertAreaPerTime), 'LineWidth', 2, 'Color', colors(numBins-b+1,:))
%     hold on
%     plot(-1*shapeMean.retractionFront(numBins-b+1,:)*convertAreaPerTime/min(shapeMean.retractionFront(numBins-b+1,:)*convertAreaPerTime), 'LineWidth', 2, 'Color', colors(numBins-b+1,:))   
% end
% colorbar;
% title('Normalized (by maxs) protrusive and retractive motion as a function of cell orientation with respect to the lines');
% xlabel('boundary position (a.u.)');
% ylabel('normalized local motion (square micrometers/minute)')
...
colors = colormap(hsv(numBins)); %colors = colormap(cool(2*numBins+1));
for b=1:numBins
    plot(shapeMean.protrusionFront(b,:)*convertAreaPerTime, 'LineWidth', 2, 'Color', colors(numBins-b+1,:))
    hold on
    plot(fliplr(-1*shapeMean.retractionFront(b,:)*convertAreaPerTime), 'LineStyle', ':', 'LineWidth', 2, 'Color', colors(numBins-b+1,:))   
end
colorbar;
title('Protrusive and retractive motion as a function of cell orientation with respect to the lines');
xlabel('boundary position (a.u.)');
...
colors = colormap(hsv(numBins));
for b=1:numBins
    plot(shapeMean.protrusionFront(b,:)*convertAreaPerTime-fliplr(-1*shapeMean.retractionFront(b,:)*convertAreaPerTime), 'LineWidth', 2, 'Color', colors(numBins-b+1,:))
    hold on  
end
grid on
colorbar;
title('Protrusive - retractive motion as a function of cell orientation with respect to the lines');
xlabel('boundary position (a.u.)');
...
% bins = (pi/4)/numBins:(pi/2)/numBins:(pi/2-(pi/4)/numBins);
...
% [markerColor, marker, markerSize, lineSep] = findColor(inDirectory);
% %figure
...
%     %plot(angleToFit, proToFit(r,:), 'LineWidth', 2, 'Marker', marker, 'MarkerSize', markerSize, 'Color',markerColor);
%     %line(angleToFit, oneVonMises(coefEsts(1,r), angleToFit), 'LineWidth', 1.5, 'Color','r');
%     %hold on
% end
% % figure
% %for r=1:proToFitRows
% %     plot(bins,coefEsts, 'LineWidth', 2, 'Marker', marker, 'MarkerSize', markerSize, 'Color', markerColor);
% %     hold on
% %end
% plot(1/lineSep, (coefEsts(2)-coefEsts(1))/(pi/4), 'LineWidth', 2, 'Marker', marker, 'MarkerSize', markerSize, 'Color', markerColor);
% hold on
...
%offsetStart = [1,1,1];
%coefExp = nlinfit(eccMean, coefEsts, offsetExp, offsetStart);
...
% coefEsts
% %figure
% plot(bins,coefEsts, 'Marker', '.', 'MarkerSize', 20,  'Color', 'b', 'LineStyle', 'none')

% 
% shapeGauss= [fliplr(shapeMean.protrusionFront(1,:)*convertAreaPerTime), shapeMean.protrusionFront(1,:)*convertAreaPerTime];
% shapeGauss = shapeGauss/(sum(shapeGauss))*160/(2*pi);
% angles = -pi+((2*pi)/320):((2*pi)/160):pi-((2*pi)/320);
% save([savePath 'shapeGauss'],'shapeGauss');
% save([savePath 'angles'],'angles');

% find the color and marker type of the run
function [markerColor, marker, markerSize, lineSep] = findColor(inDirectory)
